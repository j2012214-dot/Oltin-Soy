            ...
            with st.form("admin_full_reg_form"):
                ...
                if st.form_submit_button("Зарегистрировать и заселить"):
                    if not adm_name:
                        ...
                        send_telegram_notification(f"🏨 *Рустам зарегистрировал:* {adm_name}\n🛏 Палата: {room_field}\n👥 Гостей: {adm_guests}\n📅 До: {out_date_str}\n💰 Сумма: {cost:,} сум")
                ...
                if not av_rooms.empty:
                    ...
                    with st.form("client_book"):
                        ...
                        if st.form_submit_button("Забронировать"):
                            ...
                            ''', (f"{r_num} ({r_type})", str(in_d), str(out_d), days, int(gst), cost, current_user['phone']), is_select=False)
                            run_query("UPDATE rooms SET status='Занято' WHERE room_number=? AND price_type=?", (r_num, r_type), is_select=False)
                            
                            send_telegram_notification(f"🏨 *Бронь от клиента:* {current_user['name']}\nПалата: {r_num}\nДо: {out_d}")
