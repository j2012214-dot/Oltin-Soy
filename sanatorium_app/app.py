...
# Очистка аномальных строк со множественными запятыми (например, "1,0,0,0,0" -> 1)
def clean_numeric(val):
    if val is None:
        return 0
    s = str(val).strip()
    if ',' in s:
        # Если строка вида "1,0,0,0,0", берем только первую часть до запятой
        s = s.split(',')[0]
...
def init_db():
    ...
    # Добавляем недостающие колонки по отдельности (защищено от сбоев "duplicate column name")
    columns_to_add = {
        "guests_count": "INTEGER DEFAULT 1",
        "birth_year": "TEXT DEFAULT '-'",
        "passport_series": "TEXT DEFAULT '-'",
    ...
    # Добавляем администратора Рустама
    ...
    # Заполнение палат по умолчанию, если таблица пуста
...
# Глобальные сессии для Streamlit
            ...
            if not p_phone:
                ...
                if check_user is not None and not check_user.empty:
                    ...
                    if user_res is not None and not user_res.empty:
                        ...
                        send_telegram_notification(f"🔔 *Новый клиент зарегистрировался сам!*\n👤 Имя: {new_name}\n📱 Номер: {p_phone}\n📄 Документ: {new_id_type} ({new_passport})")
...
# ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # ИНТЕРФЕЙС АДМИНИСТРАТОРА (РУСТАМ)
    # ---------------------------------------------------------
    if role == 'Admin':
        ...
        with t1:
            st.subheader("Текущие пациенты и управление бронированием")
            ...
            if all_patients is not None and not all_patients.empty:
                ...
                st.info("Нет активных проживающих пациентов.")
        ...
        with t2:
            ...
            with st.form("admin_reg_form"):
                ...
                if st.form_submit_button("Зарегистрировать пациента"):
                    if p_name and p_phone and p_birth and p_passport:
                        ...
                        send_telegram_notification(f"➕ *Рустам добавил пациента вручную:*\n👤 ФИО: {p_name}\n📱 Тел: {p_phone}\n📦 Пакет: {p_package}")
                        
                        st.success("Пациент успешно добавлен в систему!")
                        st.rerun()
            
            st.markdown("---")
            st.subheader("🗑️ Удаление аккаунтов клиентов")
            ...
            if all_db_patients is not None and not all_db_patients.empty:
                ...
                if st.button("❌ Удалить выбранного клиента навсегда", use_container_width=True, type="primary"):
                    ...
                    st.success(f"Аккаунт пациента {selected_del_phone} полностью удален.")
                    st.rerun()

        with t3:
            st.subheader("Состояние номерного фонда и цены")
            rooms_df = run_query("SELECT room_number as 'Номер комнаты', price_type as 'Тип палаты', price_per_day as 'Цена за день (сум)', corpus as 'Корпус', floor as 'Этаж', status as 'Статус' FROM rooms")
            ...
            with st.form("appointment_form_fixed"):
                ...
                if submit_btn:
                    if chosen_patient != "Нет пациентов":
                        ...
                        st.error("Список пациентов пуст.")
                ...
                if st.button("🚪 Оформить выписку и отправить в архив", use_container_width=True):
                    ...
                    if p_res is not None and not p_res.empty:
                        ...
                        run_query('''
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ''', (p_data['name'], p_data['phone'], p_data['room'], p_data['check_in_date'], str(pure_days), str(pure_guests), str(pure_cost), p_data['payment_method'], str(datetime.now().date())), is_select=False)
                        ...
                        send_telegram_notification(f"📤 *Пациент успешно выписан:*\n👤 ФИО: {p_data['name']}\n🏨 Из палаты: {p_data['room']}\n💰 Сумма: {pure_cost:,} сум")
        ...
        with t6:
            ...
            if archive_selector_df is not None and not archive_selector_df.empty:
                selected_archive_row = st.selectbox(
                    ...
                    [f"{row['name']} ({clean_numeric(row['total_cost']):,} сум) от {row['archive_date']} [ID:{row['id']}]" for _, row in archive_selector_df.iterrows()]
                )
                if st.button("🗑️ Удалить запись из отчетов", type="primary"):
                    ...
                    st.success("Запись удалена из отчетов.")
                    st.rerun()
            ...
            if full_archive is not None and not full_archive.empty:
                # Очищаем данные перед показом, чтобы убрать баг с кучей нулей и запятых
    ...
    # ---------------------------------------------------------
        ...
        with p_tab1:
            if current_user['room'] == '-':
                available_rooms = run_query("SELECT room_number, price_type, price_per_day, corpus FROM rooms WHERE status='Свободно'")
                ...
                if available_rooms is not None and not available_rooms.empty:
                    ...
                    with st.form("book_room"):
                        ...
                        check_in_date = st.date_input("Выберите дату заезда")
                        ...
                        if st.form_submit_button("Подтвердить бронирование"):
                            if selected_room_idx:
                                ...
                                if room_info is not None and not room_info.empty:
                                    ...
                                    send_telegram_notification(f"🏨 *Новое бронирование палаты!*\n👤 Клиент: {current_user['name']}\n📱 Тел: {current_user['phone']}\n🛏 Палата: {r_num} ({r_type})\n⏳ Срок: {duration} дней")
                                    st.success(f"🎉 Палата {r_num} успешно забронирована!")
                                    st.rerun()
                else:
                    st.info("В данный момент все палаты заняты.")
            else:
        ...
        with p_tab2:
            ...
            if my_appointments is not None and not my_appointments.empty:
                ...
                st.info("Назначенных процедур пока нет.")
